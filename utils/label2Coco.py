...
class_list_1 = {
    ...
    'id': 0,
    'name': 'pin',
    'keypoints': ['pin_start', 'pin_end'],  # 大小写敏感
    'skeleton': [[0, 1]]
}
class_list_2 = {
    'supercategory': 'num',
...
def label2coco(indx=1):
    def process_single_json(labelme, image_id=1):
        ...
        for each_ann in labelme['shapes']:  # 遍历该json文件中的所有标注
            ...
            if each_ann['shape_type'] == 'rectangle':  # 筛选出个体框
                ...
                for each_class in class_list_2['keypoints']:
                    if each_class in bbox_keypoints_dict:
                        bbox_dict['keypoints'].append(bbox_keypoints_dict[each_class][0])
                        bbox_dict['keypoints'].append(bbox_keypoints_dict[each_class][1])
                        bbox_dict['keypoints'].append(2)  # 2-可见不遮挡 1-遮挡 0-没有点
                        bbox_dict['category_id'] = 2  # 此处需要更改ID
                    else:  # 不存在的点，一律为0
                        bbox_dict['keypoints'].append(0)
                        bbox_dict['keypoints'].append(0)
                        bbox_dict['keypoints'].append(0)
    ...
    import json
    import numpy as np
    Dataset_root = r'D:\AI_fileCollected\code\Measurement\Measurement\mmdetection\data\Measurement'
    coco['info'] = {}
...
if __name__ == '__main__':
    # dataSeperate()
    label2coco(1) # 1 为训练集 2 为测试机
    # 得到完整的MS COCO格式的数据集
    from pycocotools.coco import COCO
    my_coco = COCO(r'/data/Measurement\train_coco.json')
    # my_coco = COCO(r'D:\AI_fileCollected\code\Measurement\Measurement\Datasets\Measurement\val_coco.json')