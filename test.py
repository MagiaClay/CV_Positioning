...
def pic_predic(img_path, detector, pose_estimator):
    ...
    model_value_temp = '%.2f' % model_value
    ...
    # 5. 将结果显示并绘制在图片上
    cv2.putText(img_bgr, f'value:{model_value_temp},error:{error_temp}', (0, 1500), cv2.FONT_HERSHEY_SIMPLEX, 8, (0, 0, 255), 7)
    cv2.circle(img_bgr, point_left, kpt_radius, kpt_color, -1)
    cv2.circle(img_bgr, point_right, kpt_radius, kpt_color, -1)
    return img_bgr, model_value_temp, error
...
if __name__ == '__main__':
    ...
    imagelist = os.listdir(r'D:\AI_fileCollected\Datasets\Measurement\test_3_outter/')
    rootdir = r'D:\AI_fileCollected\Datasets\Measurement\test_3_outter/'
    errors = []
    for i in range(len(imagelist)):
        img_path = rootdir + imagelist[i]
        frame,value, error = pic_predic(img_path, detector, pose_estimator)
        error_temp = '%.3f' % error
        cv2.imwrite(f'D:\AI_fileCollected\Datasets\Measurement/test_3_outter_result/' + imagelist[i] + '_' + value + '.jpg', frame)
        if error < 0.05:  # 排除异常值干扰提高效率
            errors.append(error)
    average_error = sum(errors) / len(errors)
    average_error = '%.3f' % average_error
    print(f'平均误差为：{average_error}')
